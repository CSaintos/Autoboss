...
void Product_db::addProduct(Warehouse wh, Product prod, int quantity) {
	helper.sqlexec("UPDATE Products SET quantityInStock = (quantityInStock + " + std::to_string(quantity) + 
	") WHERE productID = " + std::to_string(prod.getProductID()) + " and warehouseID = " + std::to_string(wh.getWarehouseID()) + ";");
